...
const transform: AxiosTransform = {
  ...
  beforeRequestHook: (config, options) => {
    ...
    // update-begin--author:auto---date:2025---for：当请求地址包含saveOrUpdate时，自动添加orgCode参数
    // 检查是否是POST请求且URL包含saveOrUpdate
    if (config.method?.toUpperCase() === RequestEnum.POST && requestUrl && requestUrl.includes('saveOrUpdate')) {
      const userStore = useUserStoreWithOut();
      const currentEnterpriseCode = userStore.currentEnterpriseCode;
      
      // 如果存在currentEnterpriseCode，则自动添加orgCode参数
      if (currentEnterpriseCode) {
        // 处理data参数（POST请求的数据通常在data中）
        if (config.data && typeof config.data === 'object' && !Array.isArray(config.data) && !isString(config.data)) {
          // 如果data中没有orgCode，则添加
          if (!config.data.hasOwnProperty('orgCode')) {
            config.data.orgCode = currentEnterpriseCode;
          }
        }
        // 处理params参数（某些情况下POST请求的参数可能在params中）
        if (config.params && typeof config.params === 'object' && !Array.isArray(config.params) && !isString(config.params)) {
          if (!config.params.hasOwnProperty('orgCode')) {
            config.params.orgCode = currentEnterpriseCode;
          }
        }
      }
    }
    // update-end--author:auto---date:2025---for：当请求地址包含saveOrUpdate时，自动添加orgCode参数
