...
export function useListPage(options: ListPageOptions) {
  ...
  // 监听 currentEnterpriseCode 变化，自动刷新表格（仅在 watchEnterpriseCode 为 true 时）
  const watchEnterpriseCode = options.tableProps.watchEnterpriseCode !== false; // 默认为 true
  ...
  async function onExportXls() {
    ...
    if (realUrl) {
      ...
      // 自动添加 orgCode 参数（仅在 autoOrgCode 为 true 时）
      const autoOrgCode = options.tableProps.autoOrgCode !== false; // 默认为 true
      if (autoOrgCode && userStore.currentEnterpriseCode) {
        paramsForm.orgCode = userStore.currentEnterpriseCode;
      }
...
export function useListTable(tableProps: TableProps): [
  ...
  function beforeFetch(params) {
    // 默认以 createTime 降序排序
    const result = Object.assign({ column: 'createTime', order: 'desc' }, params);
    // 自动添加 orgCode 参数
    if (userStore.currentEnterpriseCode && defaultTableProps.autoOrgCode) {
  ...
  if (typeof tableProps.beforeFetch === 'function') {
    const userCustomBeforeFetch = tableProps.beforeFetch;
    defaultTableProps.beforeFetch = function (params) {
      // 先执行默认逻辑（添加排序和 orgCode）
      params = beforeFetch(params);
      // 再执行用户自定义逻辑
      // @ts-ignore
      const customResult = userCustomBeforeFetch(params);
      return customResult || params;
    };
  }
