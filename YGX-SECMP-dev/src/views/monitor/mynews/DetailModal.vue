<template>
  <BasicModal
    ...
    :destroyOnClose="true"
  >
    <a-card class="daily-article" :bodyStyle="{ backgroundColor: backgroundColor, height: '100%' }" :headStyle="{ backgroundColor: backgroundColor }">
      ...
      <div v-if="content.fileJson" class="flex">
        附件：
        <JUpload v-model:value="content.fileJson" :disabled="true"></JUpload>
        <!-- <div v-for="file in fileList"  :key="file.id" class="ml-[20px]" >
          <img :src="file.path" width="100" height="100" alt="file.fileName" />
        </div> -->
...
<script lang="ts" setup>
  ...
  const backgroundColor = computed(() => {
    const mode = getDarkMode.value;
    return mode === ThemeEnum.DARK ? '#0b1c36' : '#ffffff';
  });
  ...
  const fileList = computed(() => {
    if (content.value.fileJson) {
      return JSON.parse(content.value.fileJson);
    }
    return [];
  });
