<template>
    ...
    >
      <div class="h-full m-t-10" v-loading="state.loading">
        ...
        >
          <a-row>
            <a-col :span="24">
              <a-form-item label="制度名称" name="name" :rules="[{ required: true, message: '请输入制度名称' }]">
                <a-input v-model:value="state.formState.name" placeholder="请输入" allowClear />
              </a-form-item>
            </a-col>
            <a-col :span="24">
              <a-form-item label="制度描述" name="memo" :rules="[{ required: true, message: '请输入制度描述' }]">
  ...
  import { saveOrUpdateManagement, getManagementViewById } from '../line.api';
  ...
  const getData = async () => {
    state.loading = true;
    try {
      const result = await getManagementViewById({ id: row.value.id });
  ...
  const handleOk = () => {
    formRef.value.validate().then(async () => {
      ...
      try {
        ...
        await saveOrUpdateManagement(params);
        emit('success');
