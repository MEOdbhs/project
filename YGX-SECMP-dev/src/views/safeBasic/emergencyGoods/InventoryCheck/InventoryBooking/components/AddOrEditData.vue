    ...
    <div class="h-full" v-loading="state.loading">
      <a-form ref="formRef" :model="state.formState" :label-col="labelCol" :wrapper-col="wrapperCol" :disabled="readonly" autocomplete="off">
        <a-row :gutter="24">
          ...
          <a-col :span="24">
            <div class="public-title">审核记录</div>
          </a-col>

          <a-col :span="24">
            <a-table
              class="review-table-center"
            ...
            >
              <template #bodyCell="{ column, record }">
                <template v-if="column.dataIndex === 'reviewStatus'">
                  <a-tag :color="isReviewPassed(record.reviewStatus) ? 'green' : 'red'">
                    {{ isReviewPassed(record.reviewStatus) ? '通过' : '驳回' }}
                  </a-tag>
                </template>
              </template>
            </a-table>
...
<script setup lang="ts">
  ...
  import { getStorageList, saveOrUpdateResult, getViewById, saveOrUpdateReview } from '../line.api';
  ...
  const reviewColumns = [
    { title: '序号', dataIndex: 'index', key: 'index', align: 'center', customRender: ({ index }) => index + 1 },
    { title: '审核人', dataIndex: 'reviewUserName', key: 'reviewUserName', align: 'center' },
    { title: '审核时间', dataIndex: 'createDt', key: 'createDt', align: 'center' },
    { title: '审核状态', dataIndex: 'reviewStatus', key: 'reviewStatus', align: 'center' },
    { title: '备注', dataIndex: 'reviewRemark', key: 'reviewRemark', align: 'center' },
  ];

  // 当前仓库物资数据
  const goodsTableData = ref<any[]>([]);
  const goodsAllData = ref<any[]>([]); //总数据
  const reviewTableData = ref<any[]>([]); //审核数据
  ...
  const getData = async () => {
    ...
    if (data) {
      ...
      reviewTableData.value = (data.reviewList || []).map((item, index) => ({
        ...item,
        id: item.id || `${index}`,
  ...
  const isReviewPassed = (status) => {
    return ['1', 1, '通过', 'pass', 'PASS', true].includes(status);
  };
...
<style>
  ...
  .review-table-center .ant-table-thead > tr > th,
  .review-table-center .ant-table-tbody > tr > td {
    text-align: center;
  }
  .public-title {
    font-size: 16px;
