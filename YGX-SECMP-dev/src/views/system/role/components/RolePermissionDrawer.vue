<template>
  <BasicDrawer v-bind="$attrs" @register="registerDrawer" width="650px" destroyOnClose showFooter>
    ...
    <BasicTree
      ...
      :checkStrictly="true"
      :filterFn="permissionTreeFilterFn"
      title="所拥有的的权限"
      @check="onCheck"
...
<script lang="ts" setup>
  ...
  * 翻译菜单名称
   */
  /** 权限树模糊搜索：匹配 title / slotTitle（不区分大小写） */
  function permissionTreeFilterFn(searchValue: string, node: Recordable) {
    const q = (searchValue ?? '').trim().toLowerCase();
    if (!q) return true;
    const text = String(node.title ?? node.slotTitle ?? '').toLowerCase();
  ...
  function reset() {
    ...
    roleId.value = '';
    nextTick(() => {
      unref(treeRef as any)?.setSearchValue?.('');
    });
  }
  /**
   * 获取tree实例
