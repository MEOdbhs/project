    ...
    <BasicTable @register="registerTable">
      ...
      <template #status="{ record }">
        <a-tooltip placement="topLeft">
          ...
          <a-switch :disabled="record.username === 'admin'" :checked="record.status == 1" @change="handleStatusChange(record)" />
        </a-tooltip>
      </template>
      <!--操作栏-->
...
<script lang="ts" name="system-user" setup>
  ...
  //注册table数据
  const [registerTable, { reload, updateTableDataRecord, getDataSource }, { rowSelection, selectedRows, selectedRowKeys }] = tableContext;

  // 设置表格高度占满剩余高度
  useTableHeight(getDataSource);
  ...
  const handleStatusChange = (record) => {
    handleFrozen(record, record.status == 1 ? 0 : 1);
  };
