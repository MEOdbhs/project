<template>
    <a-card :bordered="false" class="risk-distribution-card">
        <template #title>
...
import { getRiskLevel } from '../line.api'
...
const averageRiskCount = ref(0)
const largerRiskCount = ref(0)
const majorRiskCount = ref(0)

const total = computed(() => lowRiskCount.value + averageRiskCount.value + largerRiskCount.value + majorRiskCount.value)

const chartData = computed(() => [
    { name: '低风险', value: lowRiskCount.value, color: '#1890ff' },
    { name: '一般风险', value: averageRiskCount.value, color: '#ffc53d' },
    { name: '较大风险', value: largerRiskCount.value, color: '#ff7a45' },
    { name: '重大风险', value: majorRiskCount.value, color: '#ff4d4f' },
])
...
const getRiskLevelData = async () => {
    ...
    const res = await getRiskLevel(params)
    if (res) {
        lowRiskCount.value = res.lowRiskCount || 0
        averageRiskCount.value = res.averageRiskCount || 0
        largerRiskCount.value = res.largerRiskCount || 0
        majorRiskCount.value = res.majorRiskCount || 0
    }
    nextTick(()=> {
        renderChart()
    })
