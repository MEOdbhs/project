...
type SaveStrategy = 'baseInfo' | 'parentCompany' | 'detailProperty' | 'safeLicence'
...
const ALL_TABS: { key: string; label: string; showFor?: string[] }[] = [
  { key: 'baseInfoDTO', label: '钢铁专项信息' },
  { key: 'parentCompanyDTO', label: '粉尘涉爆专项信息' },
...
async function activeTabClick(value) {
  ...
  if (value === 'parentCompanyDTO' && props.detail?.id) {
    try {
      const res = await getDetailCompany({ steelId: props.detail.id })
      if (res) {
        emit('detailUpdated', {
          ...props.detail,
...
async function handleSave() {
  ...
  try {
    ...
    if (strategy === 'baseInfo') {
      ...
      await addEnterprise({ steelId: props.detail?.id, ...formData })
      ...
      await saveParentCompany({ steelId: props.detail?.id, ...formData })
      message.success('粉尘涉爆专项信息保存成功')
