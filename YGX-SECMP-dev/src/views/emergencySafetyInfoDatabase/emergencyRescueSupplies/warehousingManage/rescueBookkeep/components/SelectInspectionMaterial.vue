...
import { listPage } from '../../rescueCheck/check.api';
import { BasicTable } from '/@/components/Table';
import { useListPage } from '/@/hooks/system/useListPage';
import { bookkeepColumns, searchBookkeepFormSchema } from '../bookkeep.data';
...
const { tableContext } = useListPage({
  ...
  tableProps: {
    api: listPage,
    immediate: false,
    columns: bookkeepColumns,
    bordered: true, //默认是true，可不设置
    showActionColumn: false,
    rowSelection: { type: 'checkbox' },
    showTableSetting: false,
    rowKey: 'warehouseEntryId',
    beforeFetch: (params) => {
      params.testResults = 'Y';
    },
    formConfig: {
      labelWidth: 80,
      rowProps: { gutter: 24 },
      schemas: searchBookkeepFormSchema,
      autoAdvancedCol: 4,
