...
<template>
    <BasicTable @register="registerTable">
        <template #tableTitle>
            <a-button v-auth="'rescueWarehousing:add'" type="primary" preIcon="ant-design:plus-outlined" @click="handleAdd">新增</a-button>
        </template>
...
import { columns, searchFormSchema } from './rescueWarehousing.data'
import { listPage, deleteFlagById } from './rescueWarehousing.api'
...
const { tableContext } = useListPage({
    ...
    tableProps: {
        ...
        formConfig: {
            ...
            autoAdvancedCol:4,
            fieldMapToTime: [['warehouseEntryDate', ['startWarehouseEntryDate', 'endWarehouseEntryDate'], 'YYYY-MM-DD']],
        },
        actionColumn: {
            width: 200,
        },
...
function getTableAction(record) {
    return [
        ...
        },
        {
            ...
            auth:'rescueWarehousing:edit',
            ifShow:() => record.testResults === 'S',
        },
        {
            label: '删除',
            auth:'rescueWarehousing:delete',
            ifShow:() => record.testResults === 'S',
            popConfirm: {
                title: '是否确认删除',
