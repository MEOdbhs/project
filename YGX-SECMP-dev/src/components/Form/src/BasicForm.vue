<template>
  <Form v-bind="getBindValue" :class="getFormClass" ref="formElRef" :model="formModel" @keypress.enter="handleEnterPress">
    <Row>
      <slot name="formHeaderForTable"></slot>
    </Row>
    <Row v-bind="getRow">
    ...
    <img
      v-if="Object.keys($slots).includes('formHeaderForTable')"
      src="@/assets/business/img/back-left.webp"
      class="position-absolute left-0 top-46px w-8px"
    />
  </Form>
</template>
<script lang="ts">
      ...
      const getFormClass = computed(() => {
        return [
          prefixCls,
          {
            ...
            'jeecg-form-detail-effect': unref(getProps).disabled && unref(getProps).needHideBorder,
          },
        ];
      });
