...
<template>
  <div>
    <BasicModal v-bind="$attrs" @register="register" :title="modalTitle" :width="showSelected ? '1200px' : '900px'"
      wrapClassName="j-user-select-modal" @ok="handleOk" @cancel="handleCancel" :maxHeight="maxHeight" :centered="true"
      ...
      <a-row>
        <a-col :span="showSelected ? 18 : 24">
          <BasicTable ref="tableRef" :columns="columns" :scroll="tableScroll" v-bind="getBindValue"
            :useSearchForm="true" :formConfig="formConfig" :api="getOrgUserList" :searchInfo="searchInfo"
            :rowSelection="rowSelection" :indexColumnProps="indexColumnProps" :afterFetch="afterFetch"
        ...
        <a-col :span="showSelected ? 6 : 0">
          <BasicTable v-bind="selectedTable" :dataSource="selectRows" :useSearchForm="true"
...
import { getOrgUserList } from '/@/api/common/api';
...
export default defineComponent({
  ...
  setup(props, { emit, refs }) {
    ...
    const orgCode = ref(userStore.currentEnterpriseCode || '');
    ...
    // 包装 getOrgUserList，确保所有调用都包含 orgCode 参数
    const wrappedGetOrgUserList = (params) => {
      const paramsWithOrgCode = Object.assign({ orgCode: unref(orgCode) }, params);
      return getOrgUserList(paramsWithOrgCode);
    };
    ...
    /**
     * 请求之前根据创建时间排序，并添加orgCode参数
     *
     * @param params
     */
    function beforeFetch(params) {
      return Object.assign({ column: 'createTime', order: 'desc', orgCode: unref(orgCode) }, params);
    }
